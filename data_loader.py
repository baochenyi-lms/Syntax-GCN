...
class DataLoader(object):
    def __init__(self, filename, batch_size, args, vocab):
        ...
        # preprocess data
        data = self._preprocess(data, vocab)
    ...
    def _preprocess(self, data, vocab):
        ...
        for d in data:
            for aspect in d['aspects']:
                ...
                tok = [token_vocab.stoi.get(t, token_vocab.unk_index) for t in tok]     # mapping token
                asp = [token_vocab.stoi.get(t, token_vocab.unk_index) for t in asp]     # mapping aspect
                label = pol_vocab.stoi.get(label)                                       # mapping label
                pos = [pos_vocab.stoi.get(t, pos_vocab.unk_index) for t in pos]         # mapping pos
                head = [int(x) for x in head]                                           # mapping head to int
                assert any([x == 0 for x in head])
                deprel = [dep_vocab.stoi.get(t, dep_vocab.unk_index) for t in deprel]   # mapping deprel 
                post = [post_vocab.stoi.get(t, post_vocab.unk_index) for t in post]     # mapping post
